...
const Navbar = () => {
  ...
  // ── Issue #5: hide Dashboard button on profile page ──
  ...
  const menuItems = [
    // Only show Dashboard option in dropdown if NOT on profile page
    ...(!isProfilePage ? [{ key: 'dashboard', label: 'Dashboard', icon: <DashboardOutlined />, onClick: () => navigate('/dashboard', { state: location.state }) }] : []),
    ...
    <>
      <nav style={{
        ...
        <div onClick={handleLogoClick} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
          <img src="/DRAINZERO-LOGO.png" alt="DrainZero" style={{ height: 32, width: 'auto' }}
        ...
        {user && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }} className="navbar-desktop">
            {/* ── Hide Dashboard button on profile page (issue #5) ── */}
            {!isProfilePage && (
              <Button
